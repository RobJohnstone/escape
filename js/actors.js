var actorPrototype = $.extend({}, entityPrototype, {
	entityType: 'actor',
	hittable: true,
	health: 100,
	weapon: 'gun',
	move: function(dx, dy) {
		...
		if (dx < 0 && testX - this.halfWidth < (tile % map.columns) * tiles.tileWidth) {
			if (map.data[tileLeft] !== undefined && !tiles.tileset[map.data[tileLeft]].passable) {
				...
				testY = this.y + (dy ? (dy / Math.abs(dy)) : 0) * Math.round(this.speed * timer.coeff);	
			}
		...
		if (dy < 0 && testY - this.halfHeight < Math.floor(tile / map.rows) * tiles.tileHeight) {
			if (map.data[tileUp] !== undefined && !tiles.tileset[map.data[tileUp]].passable) {
				testY = this.halfHeight + Math.floor(tile / map.rows) * tiles.tileHeight;
				testX = this.x + (dx ? (dx / Math.abs(dx)) : 0) * Math.round(this.speed * timer.coeff);	
			}
	...
	fire: (function() {
		...
		return function(x, y) {
			var weapon = weapons[this.weapon],
				...
				ySpeed = ratio * dy,
				projectile;
			if (lastFired === undefined || lastFired < timer.time - weapon.reloadTime) {
				...
				projectile.init({
					...
					'xSpeed': xSpeed,
					'ySpeed': ySpeed,
					'firerId': this.entityId,
					'damage': weapon.damage
				});
				lastFired = timer.time;
			}
			this.direction = (dy >= 0 ? Math.PI : 0) + Math.atan(dx/(-dy));
		};
	})(),
